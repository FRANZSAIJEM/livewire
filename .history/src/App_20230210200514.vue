          ...
          <table class="table table-bordered shadow-sm">
            ...
            <tbody>
              <tr>
                ...
                <td>
                  <button @click="removeAttendance(attendance.id)" class="btn btn-success m-2 ">&check; Edit</button>
...
const removeAttendance = (id) => {
  //attendances.value = attendances.value.filter(attendance => attendance.id !== id)
...
await updateDoc(doc(collection(db,"attendance"),id), {
  capital: true
});
...
const toggleCompleted = id => {
  ...
  updateDoc(doc(collection(db,"attendance"),id),{
      completed: !attendances.value[index].completed
  }
  )};
